...
void printSerialStatus() {
  ...
  Serial.printf("T=%.1fC H=%.0f%% presence=%d security=%d sound=%d mic=%ld micPct=%d%% base=%ld thr=%ld risk=%u/%s ir=%d alarm=%d ac=%d lamp=%d lampOverride=%d page=%u ble=%d sta=%d ip=%s push=%d time=%s weather=%s out=%.1fC loc=%s rise=%s set=%s\n",
