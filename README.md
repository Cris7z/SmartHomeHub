...
联网功能默认不会把账号密码提交到仓库。需要使用 Wi-Fi、天气、Web Dashboard 或 PushPlus 时，复制示例文件：
...
如果不创建 `secrets.h`，固件仍可运行本地传感器、屏幕、按键、灯带、红外和蜂鸣器功能；Wi-Fi、天气、网页控制和 PushPlus 会保持未连接或未配置状态。
...
ILI9341 屏幕有 4 个页面，KEY4、Web Dashboard 或 BLE 命令 `page` 可以切换：
...
```text
T=26.5C H=55% presence=1 security=0 sound=0 mic=12345 micPct=19% base=10000 thr=65000 risk=25/LOW ir=0 alarm=0 ac=0 lamp=0 lampOverride=0 page=0 ble=0 sta=1 ip=192.168.1.10 push=1 time=12:30 weather=Clear out=28.5C loc=Xiamen rise=05:24 set=19:00
```
...
13. 逐个测试 5 个按键：KEY1 安防、KEY2 灯光、KEY3 空调/红外、KEY4 翻页、KEY5 清除。
14. 配置 `src/net/secrets.h` 后重启，确认串口 `[WiFi] status=CONNECTED`、`[Location] 城市 经纬度 时区` 和 `[Weather] 城市 温度 ... rise=... set=...`。
15. 切到 WEATHER 页，确认顶部时间正常，页面显示室外温度、天气、城市、日出、日落和 Wi-Fi 状态。
